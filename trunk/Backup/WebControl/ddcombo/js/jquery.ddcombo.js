...
; (function($) {
    ...
    $.DDComboBox = function(input, button, options) {
        ...
        $(button).mousedown(function(event) {
            ...
            /* 2010.12.15  JELLY修改, 清空input, 呼叫callback */
            if (input.value == '' || input.value == input.title) {
                var callback = options.fnSelectChanged;
                if (callback) {
                    setTimeout(function() {
                        callback.call(this, '');
                    }, 0);
            ...
            /* 2010.12.08 JELLY修改, 原本的click event內, 若input.value不為空, 無法顯示 */
            $(input).val("");
        ...
        function selectCurrent() {
            ...
            /* 2010.12.13 JELLY 如果有定義SelectChanged callback 就執行 */
            var callback = options.fnSelectChanged;
            if (callback) {
                setTimeout(function() {
                    var key = valueToKey($input, v);
            ...
            if (options.mustMatch) {
				...
				function(result) {
				    ...
				    if (!result) {
				        ...
				        /* 2010.12.08 JELLY修改, options.mustMatch 內, 對於不match的處理, 認為這樣比較合理 */
				        if (input.value != input.title) {
				            input.value = "";
				        }
				    }
    ...
    $.DDComboBox.main = function(obj, options) {
        ...
        var table_id = div_id + '_combotable'; /* 2010.12.08 JELLY修改邏輯, 原本為 var table_id = 'combotable'; */
        var input_id = table_id + "_input";
        var button_id = table_id + "_button";
        var title = "Type something...";

        /* 2010.12.08 JELLY由於動態產生的input無法在server端找到, 因此必須在aspx中先產生dom結構, 在執行DDComboBox.main */
        /*
        $(obj).createAppend(
        ...
        */
        ...
        $.elementReady(input_id, function() {
            $(this).ddcombo_autocomplete(button, options["options"], {
                ...
                offsetTop: 3,
                offsetLeft: -4,
                /* 2010.12.08 JELLY新增此屬性 */
                mustMatch: options.mustMatch,
                /* 2010.12.13 JELLY新增此屬性 */
                fnSelectChanged: options.fnSelectChanged
    ...
    $.DDComboBox.defaults = {
        ...
        scroll: true,
        scrollHeight: 180,
        // 2010.12.13 JELLY 新增 fnSelectChanged
        fnSelectChanged: null
    };
